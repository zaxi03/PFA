...
def check_modsec_rules(file_path):
    ...
    try:
        result = subprocess.run(['python3', script_path, "-r", file_path, "-t", tags_path, "-v", "v4.15.0-5-gabc63791"],
...
def insert_waf_rule(app, description, variables, operators, actions):
    ...
    cur.execute(
        "INSERT INTO waf_rules (rule_id, description, variables, operators, actions, origin_file) VALUES (%s, %s, %s, %s, %s, %s)",
        (current_rule_id + 1, description, variables, operators, actions,
...
def delete_waf_rule(app, rule_id):
    ...
    cur.execute("SELECT rule_id, description, variables, operators, actions FROM waf_rules WHERE rule_id = %s",
                (rule_id))
    rule = cur.fetchone()
...
def waf():
    ...
    return render_template('waf.html', nom=nom, rules=rules, apps=app_names, selected_app=selected_app, page=page,
        ...
        if form_type == 'add_app':
            ...
            if waf_manager.add_app(app_config):
                ...
                if waf_manager.reload_nginx():
                    ...
                    flash("Erreur lors du rechargement de Nginx ⚠️. (Cause probable: application non existante)",
                          "warning")
            else:
